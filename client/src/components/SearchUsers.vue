<template>
	<div class="flex flex-col relative">
		<input
			...
			type="search" v-model="q" placeholder="Search User" />
		<div v-if="q.trim().length"
...
const searchUsers = useDebounceFn(async () => {
	...
	try {
		...
		users.value = data.filter((u) => u.id !== user.value?.id)
	} catch (error) {
		console.log(error);
	}
...
const openChat = async (user_id: string) => {
	...
	try {
		...
		const chatExists = chats.value.find(chat => chat.id === data.id)
